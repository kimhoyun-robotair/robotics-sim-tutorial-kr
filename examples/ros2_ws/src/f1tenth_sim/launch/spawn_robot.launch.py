...
def generate_launch_description():
    share = get_package_share_directory('f1tenth_sim')
    lc = LaunchConfiguration
    sim_time = ParameterValue(lc('use_sim_time'), value_type=bool)
    robot_command = [FindExecutable(name='xacro'), ' ', os.path.join(share, 'urdf', 'racecar.urdf')]
    ...
    spawn = Node(package='ros_gz_sim', executable='create', output='screen',
                 arguments=['-world', lc('world_name'), '-name', 'f1tenth_sim',
