...
ROS 2 Humble과 Gazebo Classic 11에서 사용하는 교육용 C++ ModelPlugin 패키지이다.
`ground_truth_path_plugin`은 모델의 Gazebo world pose를 읽어 상대 ROS 이름
`ground_truth_path`에 `nav_msgs/msg/Path`로 발행한다.
...
삽입 가능한 Xacro 매크로는
`urdf/ground_truth_path_plugin.gazebo.xacro`에 있다. 설정과 RViz 확인,
플러그인 로딩 문제 해결은 저장소의 `docs/07_custom_plugin.md`를 참고한다.
