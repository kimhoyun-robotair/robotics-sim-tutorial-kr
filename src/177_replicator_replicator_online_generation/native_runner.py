...
def run_native(script: Path, native_args: list[str], *, headless: bool, steps: int | None) -> None:
    ...
    try:
        ...
        if script.name == "generate_shapenet.py":
            dataset = namespace["dataset"]
            if dataset.exiting:
                return
            expected = namespace["args"].num_test_images
            images = list((Path.cwd() / "_out_gen_imgs").glob("*.png"))
            if len(images) != expected:
                raise RuntimeError(f"Expected {expected} sample images, got {len(images)}")
        if not headless:
