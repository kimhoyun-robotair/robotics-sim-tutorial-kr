...
def run_native(script: Path, native_args: list[str], *, headless: bool, steps: int | None) -> None:
    ...
    try:
        ...
        if not dataset.result:
            raise RuntimeError("Native pose generator could not initialize its assets")
        if dataset.exiting:
            return
        rep = namespace["rep"]
        rep.orchestrator.wait_until_complete()
        dataset.writer.detach()
        rep.orchestrator.stop()
        if not dataset.use_s3 and not any(Path(dataset._output_folder).rglob("*.png")):
            raise RuntimeError("Generator completed without PNG data; inspect asset/writer logs")
        if not headless:
