...
z방향으로 쌓은 +x방향 광선 앞을 큐브가 y방향으로 가로지르게 하여, 광선별 차단 여부와 맞은 표면까지의 거리를 구별한다. 기본 9개 광선은 1.5 m 높이의 범위를 이산적으로 덮으며, 큐브는 힘으로 구동하지 않고 매 프레임 위치를 지정하는 static collider다. 기본 실행은 움직임에 따른 hit/miss 변화를 처음 240스텝 동안 저장한다.

## 실행 후 확인할 것

- **광선과 이동 방향**: GUI에서 `/World/Curtain`의 +x 광선들이 z방향으로 배치되고 `/World/Target`이 x=3 m, z=1 m를 유지하며 좌우로 움직이는지 본다. 떠 있는 큐브가 낙하하지 않는 것은 static collider를 직접 움직이는 이 장면의 동작이다.
- **hit 패턴의 변화**: `lightbeam.json`을 시간순으로 보며 `beam_hit`의 길이가 기본 9인지, 큐브가 y=0 부근에서 광선을 가로막을 때와 벗어날 때 값이 바뀌는지 확인한다. 마지막 행이 전부 miss여도 이전에 hit가 있었다면 자연스러운 통과 결과다.
- **맞은 광선의 거리**: `beam_hit`가 참인 위치의 `depth_m`을 골라 큐브 앞면까지 약 2.75 m인지 확인한다. hit가 없는 광선의 거리값을 실제 표면 거리로 해석하지 않는다.
- **광선 수 비교**: `--rays 3`에서는 배열 길이와 맞는 광선 개수를 비교한다. 기본 큐브의 높이와 경로에서는 중앙 광선이 계속 검출할 수 있으므로 광선 수를 줄였다고 전체 미검출 시간이 반드시 늘어나는 것은 아니다.
- **기록의 범위**: 240스텝을 저장한 뒤에도 GUI에서 큐브는 계속 왕복하지만 JSON은 추가되지 않는다. 물체가 힘을 받아 움직이는 동역학이나 안전 장비의 응답 시간을 측정한 결과로 해석하지 않는다.
...
3. `--rays 3 --output output/rays3`으로 광선 수만 줄여 배열 길이와 동시에 맞는 광선 개수를 비교합니다. curtain_length는 유지합니다. 기본 큐브는 중앙 광선에도 걸리므로 미검출 구간이 반드시 늘어나는 것은 아닙니다.
