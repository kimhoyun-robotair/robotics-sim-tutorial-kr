...
Franka의 RMPflow 설정에서 각 반응의 기여를 껐다가 단계적으로 복구해 기본 자세 편향·목표 추종·회피·방향·한도·감쇠의 역할을 구분합니다. 기본 baseline은 설치된 설정을 복사해 사용하고, 다른 phase는 선택한 항목의 가중치와 inertia를 바꾼 로컬 YAML로 실제 로봇을 구동합니다. 각 실행은 하나의 phase를 시험하므로 같은 목표·실행 길이를 유지한 여러 결과를 비교해야 합니다.

## 실행 후 확인할 것

- 실행별 `rmpflow.yaml`과 `tuning_trace.json`의 `phase`를 함께 확인합니다. baseline은 원본 설정을 사용하고 cspace부터 damping까지는 코드의 단계별 복구를 적용하며, 한 실행 중 모든 phase를 자동 순회하지 않습니다.
- `--phase cspace`에서 로봇이 기본 configuration 쪽으로 움직이고 빨간 `/World/target`을 무시할 수 있는지 봅니다. 이 단계는 위치 attractor의 가중치를 꺼 둔 비교 조건이므로 큰 `error_m`만으로 구현 실패라 판단하지 않습니다.
- target과 collision phase를 비교해 `/World/obstacle` 부근 경로가 어떻게 바뀌는지 봅니다. 앞 단계에는 회피 반응이 꺼져 있지만 고정 박스의 PhysX collision은 남아 있어 접촉하거나 움직임이 막힐 수 있습니다.
- 15스텝마다 기록된 `trace`의 `error_m`와 `joint_velocities_rad_s`를 비교해 접근 지연·잔류 오차·급격한 속도 변화를 확인합니다. `--target-gain 20`과 40 비교에서는 다른 설정과 목표를 유지하고 성능이 반드시 좋아진다고 가정하지 않습니다.
- orientation phase에서는 손의 방향도 화면에서 비교합니다. 코드가 quaternion `[0,0,1,0]`을 목표로 보내지만 JSON에는 자세 오차가 없으므로 위치 오차만으로 방향 추종을 판정하지 않습니다. sphere 표시와 작은 오차 또한 충돌·관절 한도 준수의 자동 인증은 아닙니다.
