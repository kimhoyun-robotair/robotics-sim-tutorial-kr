...
8. **File > Save As**로 `output/density-1/scene.usda`에 저장합니다. 터미널에서 다음 검사로 실제 위치·초점 거리·하향 각도를 계산합니다.
...
성공 기준은 베이크된 NavMesh, 생성된 Camera prim, 비어 있지 않은 JSON, 선택한 카메라에 대응하는 실제 관찰 시각화입니다. 목표 비율 미달과 알고리즘의 중단도 관찰 결과로 기록합니다. `inspect_stage.py`를 Script Editor에서 실행하면 단위·축·확장·NavMesh가 검사됩니다.
