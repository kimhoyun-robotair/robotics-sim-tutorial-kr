...
`--headless`에서 `--steps`를 생략하면 최대 40000번 app update 안에 끝나지 않을 때 실패합니다. GUI 기본 실행에는 이 자동 종료 상한이 없습니다. `--frames`는 이동 중의 physics frame 수가 아니라 **목적지 캡처 횟수**입니다. 9번이면 좌/우 RGB PNG 총 18개가 성공 기준이며 런처가 실제 파일 개수를 확인합니다. 출력은 이 패키지 아래 새 폴더만 허용합니다.
