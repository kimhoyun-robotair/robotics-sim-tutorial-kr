...
Franka가 큐브를 집어 옮기는 장면을 통해 Task의 장면·관찰·초기화 책임과 Controller의 동작 계산 책임을 구분합니다. 기본 `custom` 모드는 로컬 Task가 만든 파란 큐브를 (-0.3,-0.3,0.02575) m로 옮기며, `builtin` 모드는 내장 PickPlace Task의 같은 관찰 구조를 사용합니다. 집기 동작의 단계 종료와 실제 큐브 도착은 별도로 측정해, 로봇이 동작을 끝냈다는 사실만으로 성공을 판단하지 않도록 구성했습니다.

## 실행 후 확인할 것

- custom 실행의 `/World/Cube`는 한 변 0.0515 m인 파란 큐브이고 `/World/Franka` 앞 (0.3,0.3,0.3) m에서 시작합니다. 초기 낙하 후 그리퍼가 큐브를 실제로 들어 올리고 목표 쪽으로 옮기는지 봅니다.
- custom Task에서 큐브가 목표 중심의 3 cm 안에 한 번 들어가면 초록색으로 바뀝니다. 이 색은 달성 이력을 유지하므로 초록색만으로 마지막 위치도 목표 안에 있다고 단정하지 않습니다.
- `result.json`의 `controller_done`과 `cube_target_error_m`, `within_3cm`를 함께 확인합니다. 최종 위치 판정은 큐브 중심과 `target_m`의 3차원 거리 `< 0.03 m`이며, controller가 완료돼도 놓쳤다면 실제 배치 성공은 아닙니다.
- 정상 완료 뒤에는 최대 실행 길이가 허용하는 범위에서 120단계를 더 진행해 큐브가 놓인 뒤의 위치를 측정합니다. `--steps`가 먼저 끝났다면 안정화나 동작이 덜 끝난 부분 결과일 수 있습니다.
- builtin 실행은 `get_params()`의 객체 이름으로 동일한 controller 입력을 구성하는지 비교합니다. 파란색→초록색의 위 판정은 로컬 custom Task 기준이며, 두 Task의 구조 비교와 각 실행의 실제 배치 성공을 구분합니다.
...
`--steps`를 생략한 GUI 실행은 사용자가 창을 닫을 때까지 유지됩니다. 양수 `--steps N`을 지정하면 최대 N단계 실행 후 종료합니다. `--headless`에서 생략하면 기존 기본값 1800단계를 사용합니다. controller가 완료되고 120단계가 더 지나면 결과를 저장합니다. 실행 길이 제한에 먼저 도달하면 그 시점의 부분 결과를 저장합니다. `--steps`를 생략한 GUI에서는 저장 후에도 물리 시뮬레이션과 창을 유지합니다. 실행 중 GUI의 Stop/Play로 초기화를 시도하는 대신 프로그램을 다시 실행하세요. 기본 출력은 이 폴더의 `output/<고유번호>/`이며 `--output`으로 지정한 경로가 이미 있으면 덮어쓰지 않고 오류를 냅니다.
...
결과 저장 전에 창을 닫으면 동작이 중단되어 `result.json`이 작성되지 않을 수 있습니다. 큐브를 놓쳤는데 controller_done이 참일 수 있습니다. 이 controller는 이벤트 시간으로 진행하는 예제이므로 물리 성공을 별도로 확인해야 합니다. 로봇이 안 뜨면 Franka USD와 RMPflow 확장 로딩 오류를 먼저 확인합니다.
