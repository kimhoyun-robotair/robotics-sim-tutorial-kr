...
Lula가 아는 장애물 모델 안에서 Franka의 시작 관절 상태와 빨간 목표를 잇는 RRT 경로를 찾고 관절 공간 보간으로 재생합니다. `/World/wall`은 planner에 등록된 `VisualCuboid`이며 PhysX 충돌체가 아니므로, 벽 회피는 물리 접촉 반응이 아닌 계획 모델의 결과로 살펴봅니다. 기본 실행의 성공 판정은 비어 있지 않은 계획을 한 번 이상 찾았는지까지이며, 전체 재생이나 실제 목표 도달은 따로 확인해야 합니다.

## 실행 후 확인할 것

- GUI에서 `/World/panda`, 빨간 `/World/target`, `/World/wall`의 위치와 경로를 살펴봅니다. wall을 돌아가는 계획이 생성됐는지 확인하되, 로봇이 벽에서 물리적으로 멈추는 동작을 충돌 회피의 기준으로 삼지 않습니다.
- 터미널의 `RRT plan: ... interpolated actions`와 `plans.json`의 `success`, `joint_indices`, `joint_positions`를 확인합니다. `success=true`는 계획 action 목록이 있다는 뜻이며 실제 말단 오차나 재생 완료를 의미하지 않습니다.
- 고정 목표의 계획에서 action 개수를 세어 충분한 물리 스텝 동안 GUI의 움직임을 관찰합니다. `plans.json`은 전체 계획을 저장하므로 짧은 `--steps`로 중간 종료해도 완성된 목록이 남을 수 있고, 실제 관절 추종값은 이 파일에 기록되지 않습니다.
- GUI에서 target을 1 cm 넘게 옮기면 다음 60스텝 단위 확인 시 새 계획이 추가되는지 봅니다. 목표가 그대로이면 실패한 계획을 자동 재시도하지 않으며, wall만 옮겨도 재계획 조건이 충족되지는 않습니다.
- 실패 계획은 `success=false`와 빈 action 목록으로 남습니다. 유한 실행을 끝낼 때 성공 계획이 하나도 없으면 오류로 종료하며, 반복 한도를 낮췄을 때의 한 번의 실패만으로 목표가 불가능하다고 결론내리지 않습니다.
...
성공 계획이 하나 이상 있는지 JSON으로 확인하고, 충분히 오래 실행해 로봇의 재생과 wall 부근 움직임을 화면으로 살펴봅니다. 실제 추종 오차와 충돌 여유를 정량 판정하려면 별도 측정이 필요합니다. `--max-iterations`만 5000에서 100으로 낮추어 성공 여부와 계산 지연을 비교합니다. RRT는 확률적 계획기라 한 번의 실패가 목표가 불가능함을 증명하지 않습니다.
