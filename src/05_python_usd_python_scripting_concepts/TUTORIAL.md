...
같은 낙하 큐브를 독립 Python과 Script Editor에서 실행하고 시간의 주체를 구분한다. 원문은 두 실행 방식의 개념 페이지이며 실행 예제는 이 패키지에서 추가했다. 독립 실행은 `timeline.csv`를 만들고, 대화형 실행은 물리 콜백으로 얻은 높이들을 콘솔에 출력한 후 일시정지한다.
...
6. 자동으로 재생되어 120개 물리 콜백을 수집하고 멈추는지 확인한다. 스크립트가 앱의 `next_update_async()`에 제어를 양보하기 때문에 UI도 계속 반응한다.
