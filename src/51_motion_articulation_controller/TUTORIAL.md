...
4. 기본 실행은 팔의 7개 관절과 손가락 2개를 제어합니다. `--fingers-only --finger-width 0.03`은 두 손가락만 3 cm씩 엽니다. 전체 그리퍼 폭은 양쪽 이동량과 geometry를 함께 고려해야 합니다.
5. GUI 실행을 늘려 움직임을 관찰합니다. command가 한 번에 바뀌어도 링크는 물리 drive를 통해 목표로 이동합니다. 최종 absolute_error가 줄어드는지 확인합니다.
...
`joints.json`의 absolute_error가 작은 값으로 수렴하는지 확인합니다. `--finger-width`만 0.03에서 0.01로 바꾸어 비교합니다. 팔이 정지한 상태에서 두 손가락 사이 간격만 변하는 것이 기대 결과입니다.
