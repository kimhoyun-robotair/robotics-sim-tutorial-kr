...
# Modified locally: CLI lifetime controls, paused GUI updates, and guaranteed app cleanup.
"""Manually gate ROS camera publication while keeping the GUI open through Pause/Stop."""
import argparse

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('--steps', type=int, default=None,
                    help='Main-loop updates before exit, including paused GUI updates; omitted: GUI until closed, headless 1200')
...
try:
    ...
    if viewport_api is None:
        raise RuntimeError("The manual camera graph did not create a viewport/render product")
    ...
    while simulation_app.is_running() and (args.steps is None or updates < args.steps):
