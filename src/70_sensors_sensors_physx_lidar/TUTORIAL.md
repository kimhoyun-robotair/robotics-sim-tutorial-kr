...
PhysX Lidar가 렌더링된 외형이 아니라 collider를 읽는다는 사실을 두 고정 상자의 거리와 hit prim 경로로 확인한다. 대상마다 `target_0`, `target_1` class 라벨을 붙여 어느 충돌체의 반환인지 Stage와 연결하도록 했다. 기본 실행은 회전율 0의 전 방향 스캔을 사용하고, 마지막 depth·점군·각도 버퍼와 hit 경로를 저장한다.

## 실행 후 확인할 것

- **기하학적 대상**: `/World/Lidar`는 `(0,0,1)` m, `/World/Target0`과 `Target1`은 `(4,-2,1)`, `(4,2,1)` m다. `lidar.npz`의 `depth`와 `points`, `hit_prims.json`을 함께 보고 두 상자 방향의 실제 반환이 있는지 확인한다.
- **hit와 의미 라벨**: JSON의 hit 경로를 Stage의 상자 prim 및 `target_0`/`target_1` 라벨과 연결한다. 경로 배열을 정수 class ID라고 해석하거나, 빈 hit를 실제 물체 검출로 세지 않는다.
- **의도된 미검출 비교**: `--no-colliders`로 실행하면 화면에 상자가 남아도 해당 두 상자의 hit가 사라져야 한다. depth 버퍼가 존재하는 것과 실제 상자 반환이 있는 것은 별개이며, 기본 코드의 자동 검사는 버퍼가 비었는지만 확인한다.
- **회전과 관찰 시간**: 기본 `--rotation-rate 0`과 `--rotation-rate 1`을 비교한다. 후자는 한 회전에 약 1 s가 걸리므로 짧은 실행의 마지막 sweep에 두 대상이 모두 들어올 것을 전제하지 않는다.
- **거리와 저장 구간**: 거리 단위는 m이고 `depth<20` 집계는 최대거리 미만의 광선 수다. 기본 240스텝 뒤 저장되는 NPZ는 마지막 스캔 버퍼이며, 이후 GUI가 계속 스캔해도 저장된 파일은 누적 점군으로 늘어나지 않는다.
...
`--steps`를 생략하면 사용자가 창을 닫을 때까지 GUI와 물리·렌더링·센서 갱신이 계속됩니다. 처음 240스텝 뒤 마지막 스캔 버퍼와 장면을 한 번 저장하며, 이후 관찰 중에는 파일을 추가하거나 바꾸지 않습니다. `--steps N`에 양수를 주면 N스텝 뒤 스냅샷을 저장하고 종료합니다. `--headless`만 사용하면 기존과 같이 240스텝 후 종료합니다. `--interactive`는 기존 명령 호환용이며 이제 필요하지 않습니다. 명시한 `--steps`의 종료 조건을 해제하지 않고, `--headless`와 함께 사용할 수 없습니다.
