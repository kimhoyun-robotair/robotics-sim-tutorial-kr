...
parser.add_argument('--motion-bvh', action=argparse.BooleanOptionalAction, default=True)
...
app = SimulationApp({'headless': args.headless, 'enable_motion_bvh': args.motion_bvh})
try:
    import numpy as np
    import omni.usd
    ...
    (output/'measurements.json').write_text(json.dumps({'motion_bvh':args.motion_bvh,'target_speed_m_s':args.target_speed,'returns_per_frame':counts,'target_final_position':target.get_world_pose()[0].tolist(),'note':'Point clouds alone do not expose Doppler velocity'},indent=2))
