...
중력 아래의 한 관절 팔을 이용해 물리 계산 주기와 Effort Sensor의 샘플 주기가 다를 때 값과 timestamp를 어떻게 읽어야 하는지 익힌다. 같은 Y축 관절을 60 Hz 물리와 기본 10 Hz 센서 읽기로 비교하므로, 관절 토크 자체뿐 아니라 언제의 토크인지가 핵심이다. 기본 실행은 주기를 따른 sampled 읽기와 최신 physics 읽기를 같은 JSON 행에 저장한다.

## 실행 후 확인할 것

- **초기화와 유효성**: `effort.json`의 `valid=false`인 초기 행은 버리고 이후 유효한 행이 생기는지 확인한다. 설치 구현은 첫 2물리 스텝 동안 데이터 취득을 건너뛰며, 유효 행이 한 번도 없으면 코드가 실패한다.
- **두 시계**: `physics_time_s`는 약 1/60 s씩 진행하고 `sensor_time_s`는 기본 `--period 0.1`에 맞춰 갱신·유지되는지 여러 행을 비교한다. 두 timestamp가 항상 같지 않은 것은 낮은 센서 주기를 보여주는 결과다.
- **토크와 시점**: `sampled_torque_Nm`과 `latest_torque_Nm`은 `/World/Arm/Joint`의 Y축 토크(N·m)다. 초기 움직임에서 차이를 비교하고, 정지 구간에서 두 값이 비슷하다고 샘플링이 작동하지 않는 것으로 판정하지 않는다. JSON의 `valid`와 `sensor_time_s`는 sampled 읽기에 해당한다.
- **주기 변경**: `--period .2`에서 sensor timestamp가 더 긴 구간 유지되는지 확인한다. 값의 크기가 특정 배수로 변하는 것이 아니라 새 센서 시점 사이의 간격이 달라지는 것이 비교 기준이다.
- **장면과 기록의 범위**: Stage에 별도 센서 mesh가 없어도 정상이다. 이 wrapper는 관절 prim을 읽으며, 기본 240행을 저장한 뒤 GUI에서 계속 읽는 값은 파일에 추가되지 않는다.
