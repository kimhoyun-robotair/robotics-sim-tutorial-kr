...
첫 비교는 Cell Size만 0.05에서 0.025로 줄이는 것이다. 픽셀 크기가 절반이므로 같은 면적을 표현하는 격자 수가 늘고 경계가 더 촘촘해져야 한다. 계산 시간·결과 dimension도 기록한다. 두 번째 독립 비교는 Origin z만 올려 cone의 다른 단면을 보는 것이다. 여러 값을 한꺼번에 바꾸지 않는다.
