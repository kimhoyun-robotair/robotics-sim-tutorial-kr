...
3. Robot Prim/Articulation Root=`/World/jetbot`, wheel distance=`0.1125`, radius=`0.03`, Use Keyboard Control (WASD)=On으로 한다. 이름을 명시한다면 left/right 순서를 맞춘다.
...
성공은 바퀴 회전과 로봇의 실제 직진/회전이다. 한 변수 실험: 같은 v 명령에서 wheelRadius만 두 배로 하면 계산된 각속도가 절반이 되는지 Property에서 본다. 실제 자산 크기는 바뀌지 않으므로 잘못된 radius 입력은 속도 오차를 만든다. 움직이지 않으면 Play, robot prim, 관절 이름 순서, 바닥/충돌, 중복 controller를 확인한다.
