...
렌더링된 RGB 배열에 Gaussian 잡음을 더해 ROS로 보내는 과정과 원래 USD 장면의 표시를 구별한다. 회전 카메라의 동일 Render Product에서 깨끗한 영상과 가공 영상을 함께 발행해 잡음 처리의 효과를 비교한다. 기본 실행은 CPU NumPy 경로와 sigma=0.1을 사용하며, `--device cuda`는 같은 목적의 Warp 처리 경로를 선택한다.

## 실행 후 확인할 것

- `/rgb_clean`, `/rgb_augmented`의 타입이 `sensor_msgs/msg/Image`이고 실제 영상이 도착하는지 확인한다. 둘 다 640×480, `header.frame_id=sim_camera`이며 같은 카메라를 사용한다.
- RViz Image 두 개에서 가능한 한 같은 timestamp의 영상을 비교한다. 물체 배치는 같고 augmented 영상에서 밝기 잡음이 보여야 한다. 카메라가 매 스텝 0.25°씩 회전하므로 서로 다른 시각의 영상 차이를 잡음 효과로 오해하지 않는다.
- `/World/Camera`로 보는 viewport에는 잡음 없는 원래 장면이 남아야 한다. 센서 배열의 augmentation만 수행하므로 물체 재질이나 viewport까지 거칠어져야 하는 것은 아니다.
- 프로그램을 종료하고 `--sigma 0`으로 다시 실행하면 두 출력의 RGB 내용이 같아지는지 확인한다. 기본 sigma=0.1은 8-bit 밝기 표준편차 25.5에 해당하며, RGB 출력은 0–255로 clipping한 uint8이다.
- seed와 장치를 유지하고 sigma만 늘려 균일한 면의 잡음이 더 두드러지는지 비교한다. CPU와 CUDA는 난수 생성기가 달라 동일 seed라도 픽셀별 완전 일치를 성공 기준으로 삼지 않는다.
- 초기 렌더 준비 이후 여러 프레임을 수신한다. 별도 발행 Gate는 없고 시뮬레이션 렌더 간격은 1/60초지만, `ros2 topic hz`는 처리 성능과 DDS에 영향을 받는다. `--device cpu`도 장면 렌더링에는 RTX GPU를 사용한다.
