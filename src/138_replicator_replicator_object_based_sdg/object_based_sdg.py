...
def keep_gui_open(simulation_app, *, headless: bool, steps: int | None) -> None:
    """Inspect the completed scene without capturing more dataset frames."""
    if headless:
        return
    if steps is None:
        print("[SDG] Dataset saved. Close the GUI window to finish.")
    else:
        print(f"[SDG] Dataset saved. Inspecting GUI for at most {steps} updates.")
    updates = 0
    while simulation_app.is_running() and (steps is None or updates < steps):
...
config["keep_open"] = not config.get("launch_config", {}).get("headless", False) and args.steps is None
...
try:
    ...
    def run_simulation_loop(duration):
        ...
        while simulation_app.is_running() and elapsed_time <= duration:
            simulation_app.update()
    ...
    avg_frame_fps = captured_frames / wall_duration
    num_captures = captured_frames * num_cameras
    avg_capture_fps = num_captures / wall_duration
    print(
        f"[SDG] Captured {captured_frames} frames, {num_captures} entries (frames * cameras) in {wall_duration:.2f} seconds.\n"
    ...
    keep_gui_open(
        simulation_app, headless=launch_config.get("headless", False), steps=args.steps
    )
finally:
