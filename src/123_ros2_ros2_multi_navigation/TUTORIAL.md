...
세 로봇이 같은 시뮬레이션 시간을 쓰면서도 관측과 목표는 각자의 namespace로 분리해야 함을 배우는 실습이다. 세 RViz 창은 각 로봇의 지도·센서·목표 연결을 따로 확인하기 위해 사용한다. 이 폴더에는 로컬 실행기가 없으므로 Hospital 또는 Office 장면을 GUI에서 불러오고 해당 외부 Nav2 launch를 실행해야 한다. 자동 목표 세 개를 보내는 launch 작성은 수동 목표의 로봇별 도착을 확인한 뒤 수행하는 확장 단계다.

## 실행 후 확인할 것

- **환경과 로봇 대응:** 선택한 Hospital/Office 장면과 launch가 같은 환경인지 확인하고, RViz의 `Map > Topic`으로 각 창이 `/carter1`, `/carter2`, `/carter3` 중 어느 로봇에 연결됐는지 식별한다.
- **통신 분리:** `ros2 action list -t`에서 로봇별 `navigate_to_pose` 서버를, 각 `/carterN/odom`에서 해당 로봇의 상태를 확인한다. 공통 `/clock`을 공유하는 것과 odometry/action 이름이 분리되는 것을 구분한다.
- **명령 대상:** 다른 목표를 보내지 않은 상태에서 carter1 창에만 목표를 주면 carter1만 주행하고 나머지는 대기해야 한다. 같은 방법으로 carter2와 carter3를 각각 확인한 뒤 동시에 서로 다른 통로로 보낸다.
- **위치와 도착:** 각 RViz에서 scan과 지도 정렬을 확인하고, 각 로봇이 자기 목표에 실제 도착·정지하는지 Isaac Sim과 함께 본다. 세 개의 경로가 표시되는 것만으로 세 로봇 도착을 판정하지 않는다.
- **자동 목표 확장:** `three_goals.launch.py`를 만들 때는 namespace뿐 아니라 절대 토픽 `/initialpose`의 로봇별 remap도 확인한다. 목표 생성기 종료는 목표 수 소진·거절 등으로도 발생하므로 도착 결과를 따로 확인한다.
