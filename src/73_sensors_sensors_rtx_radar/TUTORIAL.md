...
움직이는 표적 앞에 OmniRadar를 만들고 render product와 annotator를 연결하여 RTX Radar 반환을 실제 배열로 얻는 과정을 익힌다. 표적의 중력을 끄고 기본 x속도 -0.5 m/s를 주어 센서 쪽 접근을 관찰하며, Motion BVH를 켜서 움직이는 geometry 정보를 렌더러에 제공한다. 기본 실행은 프레임별 반환 수와 마지막으로 얻은 비어 있지 않은 Cartesian 점군을 저장한다.

## 실행 후 확인할 것

- **표적의 움직임**: GUI에서 `/World/Target`이 초기 `(8,0,1)` m에서 센서 쪽으로 이동하는지 보고 `measurements.json`의 `target_final_position`과 대조한다. 중력을 껐으므로 낙하를 기대하지 않으며, 최종 위치는 물리 엔진의 상태다.
- **센서 구성**: `/World/Radar`의 `OmniRadar` 타입과 `OmniSensorGenericRadarWpmDmatAPI` schema, JSON의 `motion_bvh=true`를 확인한다. viewport에 별도 점군 시각화를 붙이는 코드가 없으므로 파일의 반환을 함께 확인한다.
- **반환 존재와 저장 시점**: `returns_per_frame`에 양수인 프레임이 있고 `points.npy`가 비어 있지 않은지 본다. 마지막 프레임이 0이어도 이전 점군을 저장할 수 있으며, 저장 점군의 취득 시각이 최종 표적 위치와 일치한다고 가정하지 않는다.
- **정지 비교의 범위**: `--target-speed 0`으로 만든 결과를 기본 이동 결과와 비교한다. 저장된 Cartesian 위치에는 검증된 Doppler 속도 필드가 없으므로 `target_speed_m_s`와 점군만으로 Radar 속도 추정 정확도를 평가하지 않는다.
- **긴 실행의 변화**: 기본 저장 구간은 240스텝이며 이후 GUI에서도 표적은 계속 이동한다. 표적이 센서를 지나 시야에서 사라지면 반환 감소가 가능하고, 저장된 파일은 이후 움직임을 추가 기록하지 않는다.
...
`--steps`를 생략하면 사용자가 창을 닫을 때까지 GUI와 물리·렌더링·센서 갱신이 계속됩니다. 처음 240스텝의 반환 수 이력, 마지막 비어 있지 않은 점군과 저장 시점 표적 위치를 한 번 저장하며, 이후 관찰 중에는 파일을 추가하거나 바꾸지 않습니다. `--steps N`에 양수를 주면 N스텝의 구간을 기록하고 종료합니다. `--headless`만 사용하면 기존과 같이 240스텝 후 종료합니다. `--interactive`는 기존 명령 호환용이며 이제 필요하지 않습니다. 명시한 `--steps`의 종료 조건을 해제하지 않고, `--headless`와 함께 사용할 수 없습니다.
