...
MJCF가 File > Import 형식에 없으면 extension이 꺼져 있다. 외부 MJCF의 mesh가 빠지면 XML 기준 상대 경로/asset meshdir를 확인한다. 값이 100배 다르면 metersPerUnit과 distance/중력을 맞춘다. 파일을 가져왔다고 모든 MuJoCo solver/actuator 기능이 PhysX에서 동일하게 재현되는 것은 아니다. 이 변경은 문법/CLI와 API signature만 확인했으며 GPU 변환/진자 거동은 미검증이다.
