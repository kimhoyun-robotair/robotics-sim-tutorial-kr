...
images에는 카메라별 RGB, labels에는 보이는 tracked 객체의 2D 상자, segmentation에는 객체별 마스크, descriptions에는 해당 프레임의 값이 들어간다. 가려진 큐브 때문에 라벨 줄 수가 여섯보다 적을 수 있다. 프레임 수와 RGB 파일 수를 비교한다.
