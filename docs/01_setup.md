...
ROS 2 Humble이 아직 없다면 먼저 [공식 Ubuntu 설치 절차](https://docs.ros.org/en/humble/Installation/Ubuntu-Install-Debs.html)를 따라 apt 저장소를 등록한다. 그다음 다음 패키지를 설치한다.
...
`rosdep`은 처음 사용하는 시스템에서만 초기화한다. 이미 초기화되어 있다는 메시지는 오류가 아니다.
...
빈 world가 열리면 종료한다. VM이나 원격 데스크톱에서 검은 화면이 보이면 먼저 소프트웨어 렌더링으로 원인을 분리할 수 있다.
...
마지막 출력은 반드시 `Humble`이어야 한다. `main`은 Jazzy/Gazebo Harmonic용이라 이 과정과 호환되지 않는다.
...
`--symlink-install`은 Python, launch, URDF, RViz 설정을 고친 뒤 매번 복사 설치하지 않아도 되게 한다. C++ 플러그인을 수정한 경우에는 다시 `colcon build`해야 한다.
...
source 순서는 **기본 ROS 환경 → 현재 워크스페이스 overlay**이다. 순서가 맞는지는 실제 패키지 경로로 확인한다.
...
첫 경로는 `/opt/ros/humble` 아래를, 두 번째 경로는 현재 워크스페이스의 `install/` 아래를 가리켜야 한다. 반대 순서로 source하면 기본 환경이 overlay의 패키지 경로를 가릴 수 있다.

## 7. 첫 통합 검사

GUI 없이 20초 동안 실행하면 모델 spawn과 플러그인 로딩을 빠르게 확인할 수 있다.
...
`/clock`, `/odom`, `/tf`, `/joint_states`가 보이고 TF 값이 출력되면 환경 구성이 끝난다. 다음 장에서는 모델 파일의 각 요소가 왜 필요한지 살펴본다.
...
터미널의 첫 번째 `Err` 또는 `FATAL` 줄을 찾는다. GUI 문제인지 서버/모델 문제인지 구분하려면 `gui:=false`로 `gzserver`만 실행한다. 더 자세한 진단 순서는 [문제 해결](08_debugging.md)에 정리되어 있다.
