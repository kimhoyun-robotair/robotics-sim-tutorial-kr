...
세 접점의 높이가 같아도 무게중심이 접점으로 만든 삼각형 밖에 있으면 차체가 기운다. 이 모델은 바퀴가 x=0, 뒤 캐스터가 x=-0.17 m에 있다. [앞 장의 차체 관성](05-first-robot.md)은 뒤쪽 배터리 배치를 근사해 질량중심을 x=-0.04 m로 둔다. 기본 센서를 모두 장착한 뒤에도 로봇 전체 무게중심은 x≈-0.034 m로, 세 접점 안쪽에 남는다. 센서나 배터리의 질량·위치를 바꾸면 접점의 높이와 무게중심을 함께 확인한다.
