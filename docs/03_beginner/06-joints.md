...
이 caster는 조향축과 회전축을 상세 모델링한 실제 캐스터가 아니라, 모든 방향으로 같은 형상을 가진 구를 `fixed` joint로 붙인 초급용 근사이다. `mu1`, `mu2`를 낮춰 회전할 때 caster 접촉이 차체를 과도하게 붙잡지 않도록 한다. caster는 DiffDrive의 구동 joint 목록에 넣지 않는다.

바닥 접점도 맞춰져 있다. 구동 바퀴 중심은 z=-0.06 m이고 반지름은 0.06 m이므로 바닥점은 z=-0.12 m이다. caster 중심은 z=-0.085 m이고 반지름은 0.035 m이므로 바닥점도 z=-0.12 m이다.
