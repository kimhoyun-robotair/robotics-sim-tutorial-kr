...
- 물리 계산 단계, 중력, 정적 바닥, 동적 상자의 관계를 코드에서 찾는다.
- SDF를 검사한 뒤 GUI와 서버 전용 모드로 실행한다.
- CLI 관찰값으로 월드가 제대로 올라왔는지 확인한다.

## 월드를 구성하는 네 블록

`examples/gazebo/worlds/first-world.sdf`는 외부 메시 없이 다음 네 블록만으로 실행된다.
...
`max_step_size=0.001`은 한 물리 계산의 시뮬레이션 시간 간격이 1 ms임을 뜻한다. 이상적으로 RTF가 1이면 1초에 약 1,000번의 물리 계산이 필요하다. 실제 RTF는 컴퓨터 부하에 따라 달라진다.

조명은 위치와 자세, 방향, 색, 그림자 여부를 가진다.
...
먼저 실습 1의 Gazebo를 `Ctrl+C`로 종료한다. 같은 월드를 GUI 없이 실행하면 CI나 원격 서버에서도 물리 동작을 확인할 수 있다.

```bash
gz sim -s -r examples/gazebo/worlds/first-world.sdf
```

다른 터미널에서 월드 관련 서비스와 모델 목록을 확인한다.
...
상태 토픽에서 두 표본을 읽어 두 번째 `sim_time`이 더 큰지 확인한다.

```bash
gz topic -e -t /world/first_world/stats -n 2
```

## 실습 3: 코드 한 줄을 바꾸고 결과 예측하기

실습 2의 서버를 `Ctrl+C`로 종료한다. 원본을 보존하기 위해 임시 복사본으로 실험한다.
...
Physics 시스템은 동적 `training_box`와 정적 `ground`의 collision을 이용해 접촉을 계산한다. SceneBroadcaster 시스템은 그 결과를 GUI에 전달한다. `beacon`에는 visual만 있고 `<static>true</static>`이므로 파란 표식으로만 사용된다.

## 예상 관찰

1. `gz sdf -k`가 `Valid.`를 출력한다.
2. Entity Tree와 `gz model --list`에 같은 모델 세 개가 나타난다.
3. 재생 후 `training_box`는 바닥 위에 남고 `beacon`은 움직이지 않는다.
4. Pause 상태에서는 시뮬레이션 시간과 물리 상태가 함께 멈춘다.
5. 서버 전용 모드에서도 stats 토픽과 월드 서비스를 확인할 수 있다.

## 문제 해결

- SDF 검사가 실패하면 오류 줄의 XML 요소와 닫는 태그부터 확인한다.
- `gz model --list`가 서버를 찾지 못하면 `gz sim`이 실행 중인지 확인한다.
- 서버는 시작하지만 GUI가 열리지 않으면 [문제 해결](../02_getting-started/03_troubleshooting.md)의 렌더링 항목을 확인한다.
