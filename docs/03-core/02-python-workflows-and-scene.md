# Python 실행 방식과 Scene API

같은 Isaac Sim Python API라도 **누가 애플리케이션을 시작했는가**에 따라 초기화와 반복문이 달라진다. 이 구분을 놓치면 Python import 오류, 멈춘 GUI, 초기화되지 않은 articulation 같은 문제가 생긴다.
...
| 개념 | 관리하는 대상 | 주 용도 |
|---|---|---|
| Application/Kit | 창, 렌더러, Extension, 이벤트 루프 | Isaac Sim 프로세스 전체를 실행한다. |
| Simulation context | 물리 시간 간격, 재생/일시 정지, 계산 방식 | 시간과 PhysX 계산을 관리한다. |
| `World` | simulation context, `Scene`, 작업, 콜백, logger | 로봇 실습의 전체 실행 과정을 관리하는 객체이다. |
| `Scene` | 이름으로 찾는 Core API 객체 등록 목록 | 객체 추가, 조회, 초기화 시 기본 상태 복원을 돕는다. |
| USD `Stage` | prim과 레이어를 합성한 장면 | 실제 장면 데이터의 출처이다. |

모든 Stage prim이 자동으로 `Scene` 등록 목록에 들어가는 것은 아니다. `world.scene.add(obj)`로 등록한 API 객체만 `world.scene.get_object(name)`로 찾는다. 반대로 Stage에서 prim이 삭제되었는데 Python API 객체만 들고 있으면 API 객체가 더 이상 유효하지 않을 수 있다.
...
| 실행 루프 | 스크립트가 `world.step()` 호출 | Kit 이벤트 루프에 제어권 반환 | 갱신/물리 콜백 또는 비동기 작업 |
| 긴 작업 | 동기 반복문 사용 가능 | 오래 걸리는 동기 작업은 피함 | 콜백을 짧게 유지 |
| 재실행 | 프로세스를 새로 시작 | 같은 Stage와 단일 인스턴스가 남을 수 있음 | hot reload와 `on_shutdown`의 정리 과정이 중요 |
| 적합한 일 | 일괄 실행, 테스트, headless 실행, 데이터 생성 | API 탐색, 짧은 실험 | 재사용 UI·도구·지속 실행 기능 |

### 가장 중요한 초기화 규칙

Standalone에서는 Omniverse 플러그인이 로드되기 전에 `omni`, `pxr`, 대부분의 `isaacsim.*` 런타임 모듈을 가져오면 안 된다. 다음 순서를 지킨다.
...
`from isaacsim.simulation_app import SimulationApp`도 5.1에서 사용할 수 있다. 공식 예제에는 `from isaacsim import SimulationApp` 표기도 함께 나온다. 중요한 것은 import 표기보다 **인스턴스를 만든 뒤 런타임 모듈을 불러오는 순서**이다.

## 3. Standalone 완전 예제

바로 실행하고 결과를 파일로 남기려면 [hello_stage.py](https://github.com/kimhoyun-robotair/robotics-sim-tutorial-kr/blob/IsaacSim5.1/examples/standalone/hello_stage.py)를 사용한다. 이 파일은 낙하·잔류 속도·초기화를 검사한다. 아래 코드는 같은 큐브를 만드는 흐름을 익히는 짧은 예제이다.
...
try:
    ...
    assert 0.08 < position[2] < 0.12, position
...
큐브 크기가 한 변 0.2 m이므로 중심 Z가 약 0.1 m 부근에서 멈추면 정상이다. 접촉 오프셋과 솔버 상태에 따라 마지막 소수점은 달라질 수 있으므로 지나치게 엄격한 비교를 하지 않는다.

### 재현 가능한 스텝

- 일괄 실행 실험은 고정된 `physics_dt`와 명시적인 스텝 수를 사용한다.
- 제어기는 콜백에 전달된 `step_size` 또는 설정한 물리 dt를 사용한다.
- `sleep()`으로 실제 시간을 기다리는 방식으로 물리 시간을 제어하지 않는다. `sleep(1)`은 현재 스레드를 기다리게 할 뿐 “정확히 1초의 시뮬레이션”을 보장하지 않는다.
- 렌더링이 필요 없는 물리 배치 실행에서는 `world.step(render=False)`를 사용하되 센서가 렌더를 요구하는지 확인한다.

## 4. Script Editor 비동기 예제

`Window > Script Editor`에서 다음 코드를 실행한다. 실행 중인 Kit의 메인 스레드를 `while` 루프로 점유하지 않고 비동기 대기 시점마다 이벤트 루프에 제어권을 돌려준다.
...
이 코드는 UI가 멈추고 Stop 버튼도 누를 수 없게 만들 수 있다. Script Editor에서는 비동기 API, 물리 콜백 또는 갱신 이벤트 구독을 사용한다.

## 5. Extension 수명 주기

아래 예제는 저장하지 않은 새 Stage에서 사용한다. 다른 도구가 만든 `World`를 함께 초기화하지 않도록 빈 장면에서 학습한다. 설치 경로 등록, UI, 재실행까지 포함한 설명은 [Extension과 OmniGraph](../06-developer/02-extension-and-omnigraph.md)를 따른다.
...
`config/extension.toml`의 핵심은 Python 모듈 등록이다.
...
`my/core/demo/__init__.py`에는 `from .extension import CoreDemoExtension`을 적어 클래스를 내보낸다. `extension.py`에서는 시작 시 장면 작업을 예약하고 종료 시 콜백과 작업을 해제한다.
...
Extension에서는 앱이 이미 시작되어 있으므로 `SimulationApp`을 생성하지 않는다. hot reload 때 이전 콜백, 이벤트 구독, UI 창과 비동기 작업을 정리하지 않으면 같은 제어가 두 번 실행되거나 이미 삭제된 prim을 참조한다.
...
자산 루트의 구체적인 위치를 문자열로 하드코딩하지 않고 자산 루트 경로를 찾는 API를 사용한다. 네트워크 자산 서버가 없거나 경로가 바뀔 수 있으므로 `None`과 파일 존재 여부를 검사한다.
...
`name`과 `prim_path`는 다르다. `name`은 Python `Scene`에서 찾는 고유 키이고, `prim_path`는 USD Stage 주소이다.
...
여러 로봇이나 여러 작업을 다룰 때도 prim 경로와 객체 이름이 겹치지 않도록 정한다. `/World/envs/env_0/robot`, `/World/envs/env_1/robot`처럼 환경 네임스페이스를 두면 여러 객체를 한 번에 다루는 view의 경로 패턴도 단순해진다.

## 8. 물리 콜백과 관측 기록

제어 명령은 매 물리 스텝 직전에 계산하는 편이 안정적이다.
...
이름이 같은 콜백을 다시 등록하지 않는다. 실험이 끝나면 `world.remove_physics_callback("record_cube")`로 정리한다. 무거운 파일 쓰기와 네트워크 통신은 큐에 넣고 콜백 외부의 처리 코드에서 수행한다.
...
- 장기 유지할 새 코드라면 동일 기능의 Experimental API 문서와 이전 버전에서의 전환 상태를 확인하고 작은 검증 예제로 평가한다.
- 한 모듈 안에서 기존 API 객체와 Experimental API 객체를 혼용하기 전에 호환성을 확인한다. 초기화 방법, 계산 방식과 요구하는 배열 형태가 다를 수 있다.
...
- [ ] Extension 종료 시 콜백과 비동기 작업을 정리하는 이유를 설명할 수 있다.
- [ ] `name`과 `prim_path`의 차이를 설명할 수 있다.
- [ ] Stage prim과 장면 등록 목록 객체가 일대일로 자동 연결되는 것이 아님을 이해했다.
